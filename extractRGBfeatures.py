# -*- coding: utf-8 -*-
"""
Created on Wed Oct 21 18:52:52 2020
...
def extractRGBfeature(video_path=VIDEOPATH,save_path=SAVEPATH,net=None,batch_size=64,num_flames=80):
    ...
    for i,video in enumerate(videos):
        print('%d/%d: extract %s video features' % (i,len(videos),video))
        this_features=[]
        if video.endswith('.avi'):
            v_path=os.path.join(video_path,video)
            cap=cv2.VideoCapture(v_path)
            flame_count=cap.get(cv2.CAP_PROP_FRAME_COUNT) #getting total number of frames
            if flame_count>num_flames: # sampling frames higher than required
    ...
    print('extract all video features done!')
